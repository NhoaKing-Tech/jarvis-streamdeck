...
### WHY I CHOSE OUR APPROACH INSTEAD OF TRUE DEPENDENCY INJECTION
...
We need to understand the difference between: `return wrapper` vs `return wrapper()`

This module uses two different wrapper patterns depending on how functions are called from the layouts.py file. Understanding this pattern is crucial for maintaining the StreamDeck key action system.

**PATTERN 1: Functions with Arguments -> return wrapper**

Functions that need arguments use a Closure Pattern (wrapper that captures parameters):
...
This function implements a closure pattern that returns a callable which,
...
Closure pattern - returns a function rather than executing
